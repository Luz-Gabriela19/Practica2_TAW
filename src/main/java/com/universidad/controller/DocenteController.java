...
public class DocenteController {
    ...
    public ResponseEntity<DocenteDTO> crearDocente(@Valid @RequestBody DocenteDTO docenteDTO) {
        long inicio = System.currentTimeMillis();
        logger.info("[DOCENTE] Inicio crearDocente: {}", inicio);
    ...
    public ResponseEntity<Void> eliminarDocente(@PathVariable Long id) {
        long inicio = System.currentTimeMillis();
        logger.info("[DOCENTE] Inicio eliminarDocente (físico) para ID {}: {}", id, inicio);

        docenteService.eliminarDocente(id);

        long fin = System.currentTimeMillis();
        logger.info("[DOCENTE] Fin eliminarDocente (físico) para ID {}: {} (Duración: {} ms)", id, fin, (fin - inicio));
    ...
    public ResponseEntity<DocenteDTO> obtenerPorNroEmpleado(@PathVariable String nroEmpleado) {
        long inicio = System.currentTimeMillis();
        logger.info("[DOCENTE] Inicio obtenerPorNroEmpleado para nroEmpleado {}: {}", nroEmpleado, inicio);

        DocenteDTO docente = docenteService.obtenerDocentePorNroEmpleado(nroEmpleado);

        long fin = System.currentTimeMillis();
        logger.info("[DOCENTE] Fin obtenerPorNroEmpleado para nroEmpleado {}: {} (Duración: {} ms)", nroEmpleado, fin, (fin - inicio));
